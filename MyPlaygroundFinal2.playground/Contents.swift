...
func transfer(_ transferType:String) {
    switch transferType {
    case "withdraw": if accountType == "debit" { debitWithdraw(transferAmount) } else {
        withdraw(transferAmount)}
    case "deposit": if accountType == "credit" { creditDeposit(transferAmount) } else { deposit(transferAmount)}
    default: break
    }
}
...
repeat {
    ...
    case 4: isSystemOpened = false; print("The system is closed")
    default: break
    }
}
